...
$table = function($roll = null) use(&$s, &$table) {	
	Table($s, "539", "Enslaved event", $roll ?? Roll(1, 20), [
		"1" => [ "Character escaped", function() use(&$s) {
				$s->char->enslaved = false;
				$subtable = function($subroll = null) use(&$s, &$subtable) {
					Table($s, "539E", "Escape consequence", $subroll ?? Roll(1, 8), [
						"1" => "Offered ".(Roll(1, 10) * 100)." gp reward",
						"2" => "Accompanied by ".Roll(1, 6)." slaves",
						"3" => "Government pays a bounty of ".(Roll(1, 10) * 10)." gp for escaped slaves",
		...
		"2" => [ "Willingly freed by owner", function() use(&$s) {
				$s->char->enslaved = false;
				$subtable = function($subroll = null) use(&$s, &$subtable) {
					Table($s, "539F", "Freedom consequence", $subroll ?? Roll(1, 10), [
						"1" => [ "Befriended by owner", function() use(&$s) {
								Table($s, "", "Friend type", Roll(1, 8), [
		...
		"3" => [ "Ruler of the land declared slavery illegal, got free with ".Roll(1, 100)." gp", function() use(&$s) {
				$s->char->enslaved = false;
			}],
		"4" => [ "Bought my freedom, remain as employee for ".Roll(1, 4)." year(s)", function() use(&$s) {
				$s->char->enslaved = false;
			}],
		"5" => [ "Owner dies", function() use(&$s, &$table) {
				Table($s, "539D", "Death consequence", Roll(1, 6), [
		...
		"7-8" => "Improve occupational skill by ".(Roll(1, 3) + 1)." rank(s)", /* XXX */
		...
		"12" => [ "Participates in a slave revolt", function() use(&$s, &$table) {
				...
				if($success) {
					$table(1);
					if($leader) {
						$s->char->entries[] = [ "", "", "Now have ".Roll(1, 6)." low-ability NPC follower(s)" ];
						/* XXX 761C */
					}
				} else {
		...
		"19-20" => [ "Enslaved for ".Roll(1, 4)." more year(s)", function() use(&$s, &$table) {
				$add = Roll(1, 3);
				for($i = 0; $i < $add; ++$i) {
					$table(Roll(1, 20) + 1);
				}
...
$s->char->entries[] = [ "539", "Enslaved", "for ".(Roll(1, 6))." year(s)" ];
