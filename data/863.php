	...
	Table($s, "863", "Gift or Legacy", $roll ?? Roll(1, 20), [
		...
		"4" => [ "Piece of jewelry", function() use(&$s) {
				...
				$subtable = function($roll = null) use(&$s, &$subtable) {
					Table($s, "863J", "Jewelry", $roll ?? Roll(1, 9), [
						"1" => "Amulet",
						"2" => "Necklace",
						"3" => "Earrings",
						"4" => "Tiara",
		...
		"14" => [ "Deed to a property", function() use(&$s) {
				/* NB: book says to roll d10 but has no value for 8 */
				Table($s, "863D", "Property deed", Roll(1, 9), [
		...
		"16" => [ "Piece of clothing", function() use(&$s) {
				$subtable = function($roll = null) use(&$s, &$subtable) {
					Table($s, "863C", "Piece of clothing", $roll ?? Roll(1, 8), [
						"1" => "Hat",
						"2" => "Pair of shoes",
						"3" => "Belt",
						"4" => "Cape",
						...
						"8" => [ "Part(s) of a set or costume", function() use(&$s, &$subtable) {
								$pieces = Roll(1, 4);
								for($i = 0; $i < $pieces; ++$i) $subtable(Roll(1, 7));
							}],
					]);
				};
				$subtable();
		...
		"17" => [ "Pouch of papers containing...", function() use(&$s) {
				$subtable = function($roll = null) use(&$s, &$subtable) {
					Table($s, "863P", "Piece of clothing", $roll ?? Roll(1, 10), [
						...
						"10" => [ null, function() use(&$s, &$subtable) {
								$papers = Roll(1, 3);
								for($i = 0; $i < $papers; ++$i) $subtable(Roll(1, 9));
							}],
					]);
				};
				$subtable();
		...
		"18" => [ "Sealed trunk", function() use(&$s, &$table) {
				if(Roll(1, 100) > 60) return "Sealed trunk (empty)";
				$more = Roll(1, 3) + 1;
				for($i = 0; $i < $more; ++$i) {
					do {
						$roll = Roll(1, 20);
