...
$table = function($roll = null) use(&$s, &$table) {
	Table($s, "106", "Family", $roll ?? (Roll(20) + $s->char->CuMod), [
		"-8" => "Mother and Father only",
		"9-12" => [ "Extended family", function() use(&$s) {
				$s->char->entries[] = [
					"", "Family",
					"Mother, Father, ".Roll(4)." Grandparent(s), ".Roll(4)." Aunts/Uncles/Cousins",
				];
			}],
		"13" => (Roll(2) === 1 ? "Maternal" : "Paternal")." grandparents only",
		...
		"15" => (Roll(2) === 1 ? "Maternal" : "Paternal")." aunt and uncle",
